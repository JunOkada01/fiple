...
const FittingArea: React.FC = () => {
  ...
  const handleBulkFavorite = async () => {
    ...
    try {
      // 現在のお気に入り情報を取得
      const favoriteResponse = await axios.get<FavoriteProduct[]>('http://34.201.127.158:8000/api/favorites/', {
  ...
  const handleBulkAddToCart = async () => {
    ...
    try {
      // 現在のカート情報を取得
      const cartResponse = await axios.get('http://34.201.127.158:8000/api/cart/', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      for (const item of fittingItems) {
        ...
        try {
          await axios.post(
            'http://34.201.127.158:8000/api/cart/add/',
            {
  ...
  const toggleItemFavorite = async (productId: number) => {
    ...
    try {
      if (itemFavorites[productId]) {
        ...
        if (favorite) {
          await axios.delete(`http://34.201.127.158:8000/api/favorites/delete/${favorite.id}/`, {
            headers: { Authorization: `Bearer ${token}` }
          });
            ...
            try {
              ...
              return {
                ...
                imageUrl: selectedVariant.images[0]?.image 
                  ? `http://34.201.127.158:8000/${selectedVariant.images[0].image}`
                  : item.imageUrl,
                selectedFrontImageUrl: `http://34.201.127.158:8000/${selectedVariant.front_image}`,
                selectedBackImageUrl: `http://34.201.127.158:8000/${selectedVariant.back_image}`,
              };
            } catch (error) {

...
const AllMensLeadiesKidsFilter: React.FC<AllMensLeadiesKidsFilterProps> = ({ 
  selectedGender, 
  onGenderChange 
}) => {
    return (
        <>
            ...
            <ul className="flex justify-center items-center my-8 flex-wrap text-sm md:text-base">
                <li 
                    className={`px-4 flex flex-col md:flex-row items-center border-l border-r border-gray-300 cursor-pointer ${selectedGender === '' ? 'bg-gray-100' : ''}`}
                    onClick={() => onGenderChange('')}
                >
                    <span>ALL</span>
                </li>
                <li 
                    className={`px-4 flex flex-col md:flex-row items-center border-l border-r border-gray-300 cursor-pointer ${selectedGender === '男性' ? 'bg-gray-100' : ''}`}
                    onClick={() => onGenderChange('男性')}
                >
                ...
                <li 
                    className={`px-4 flex flex-col md:flex-row items-center border-l border-r border-gray-300 cursor-pointer ${selectedGender === '女性' ? 'bg-gray-100' : ''}`}
                    onClick={() => onGenderChange('女性')}
                >
                ...
                <li 
                    className={`px-4 flex flex-col md:flex-row items-center border-l border-r border-gray-300 cursor-pointer ${selectedGender === 'その他' ? 'bg-gray-100' : ''}`}
                    onClick={() => onGenderChange('その他')}
                >

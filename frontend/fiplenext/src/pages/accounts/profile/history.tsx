...
const OrderHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrderHistory = async () => {
      try {
        const response = await axios.get<ApiResponse>('http://127.0.0.1:8000/api/orders/', {
          headers: {
    ...
    const fetchReviewedProducts = async () => {
      try {
        ...
        const response = await axios.get<number[]>('http://127.0.0.1:8000/reviews/', {
          headers: {
  ...
  const handleDeleteReview = async (productId: number) => {
    try {
      ...
      const response = await axios.delete(`http://127.0.0.1:8000/reviews/${productId}/`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });

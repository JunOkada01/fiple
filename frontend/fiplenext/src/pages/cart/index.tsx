...
const Cart: React.FC = () => {
    ...
    const fetchCartItems = async (token?: string) => {
        try {
            const response = await axios.get('http://54.221.185.90:8000/api/cart/', {
                headers: {
    ...
    const refreshToken = async () => {
        try {
            const response = await axios.post('http://54.221.185.90:8000/api/token/refresh/', {
    ...
    const handleQuantityChange = async (itemId: number, newQuantity: number) => {
        try {
            const response = await axios.patch(
                `http://54.221.185.90:8000/api/cart/${itemId}/`,
    ...
    const handleRemoveItem = async (itemId: number) => {
        try {
            await axios.delete(`http://54.221.185.90:8000/api/cart/${itemId}/delete/`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('access_token')}`
                }
            });
                                ...
                                <div className="w-full border-b border-gray-300 mx-4 flex items-center">
                                    ...
                                    <Link href={`/products/${item.product.product_origin.id}`}>
                                        <img   
                                            ...
                                            src={`http://54.221.185.90:8000/${item.product.images[0]?.image}`}

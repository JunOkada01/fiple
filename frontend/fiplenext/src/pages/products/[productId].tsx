...
const ProductDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            try {
                setLoading(true);
                const response = await axios.get(`http://34.201.127.158:8000/api/products/${productId}/`);
                setProduct(response.data);
                if (response.data.variants[0]?.images[0]) {
                setSelectedImage(`http://34.201.127.158:8000/${response.data.variants[0].images[0].image}`);
                                ...
                                <div key={colorName} className="text-center">
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            if (firstImage) {
                                                setSelectedImage(`http://34.201.127.158:8000/${firstImage.image}`);
                                            }
                                        }}
                                    ></button>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 mt-4">
                        {selectedVariants.flatMap(variant =>
                            variant.images.map(image => {
                                const imageUrl = `http://34.201.127.158:8000/${image.image}`;
                                return (
                                    <div
                                        key={image.id}
